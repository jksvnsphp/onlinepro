...
class Lunch extends MY_Controller {
	...
	public function index(){		
		...
		$data['lunch_details'] =  $this->Lunch_model->getLunch($res_id);
		
		 
	 $food_type = 2;  //  e.q breakfast / lunch / dinner
     ...
     $data['dates_in_calender']  =$this->Lunch_model->getHotelAvailabilityData($hotel_id, $food_type, '');
   
		
		$data['Title']='Update lunch';
	...
	public function add()
	{
	    ...
	    $deleteresult =  $this->Lunch_model->deletemenu($res_id );		
		$data['lunch_details'] =  $this->Lunch_model->getLunch($res_id );	
       ...
       /* $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
        ...
        if ($this->form_validation->run() == true) {*/
		   ...
		   $resulhotaltval = $this->Lunch_model->insertHotelAvailabilityData($uploaddata);	
  ...
  switch ($type) {   
    case "master_calender":
        ...
        $hotel_id = $_POST['hotel_id']; 
         $test = $this->Lunch_model->getMasterHotelAvailabilityData($hotel_id, $date);
    ...
    case "single_calender":
        ...
        $food_type = $_POST['type_id'];
            $master = $this->Lunch_model->getMasterHotelAvailabilityData($hotel_id, $date);
            $type = $this->Lunch_model->getHotelAvailabilityData($hotel_id, $food_type, $date);
