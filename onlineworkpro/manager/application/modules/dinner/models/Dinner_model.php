...
//create function for get update dinner id ----------		
   ...
   /******************************************calender********************************************/	
	...
	/*public function getHotelAvailabilityByDay($hotel_id, $food_type, $type_id) {
            // print_r($data);die;

		$query = "SELECT * FROM `bb_days_availbility`  WHERE hotel_id = :hotel_id AND food_type_id = :food_type_id AND type_id = :type_id";

		//$qh = $this->con->getQueryHandler($query, array("hotel_id" => $hotel_id,"food_type_id" => $food_type,"type_id" => $type_id));
		 $qh =$this->db->query($query)->result_array(); //print_r( $qh );	
		$data = array();		
		foreach($qh as $res){
			$data = $res;
	...
	/*public function getbufetAlacarteBytypeId($hotel_id, $type_id) {
		$query = "SELECT $type_id as `value`, type_id FROM `bb_menu_selection` WHERE `hotel_id` = $hotel_id";
