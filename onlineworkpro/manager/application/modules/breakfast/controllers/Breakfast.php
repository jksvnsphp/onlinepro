...
class Breakfast extends MY_Controller {
	...
	public function index(){		
		...
		$data['breakfast_details'] =  $this->Breakfast_model->getBreakfast($res_id);	
		

	 $food_type = 1;  //  e.q breakfast / lunch / dinner
     ...
     $data['dates_in_calender']  =$this->Breakfast_model->getHotelAvailabilityData($hotel_id, $food_type, '');
	...
	public function add()
	{
	    ...
	    $deleteresult =  $this->Breakfast_model->deletemenu($res_id );		
		$data['breakfast_details'] =  $this->Breakfast_model->getBreakfast($res_id );
		   ...
		   $resulhotaltval = $this->Breakfast_model->insertHotelAvailabilityData($uploaddata);	
  ...
  switch ($type) {   
    case "master_calender":
        ...
        $hotel_id = $_POST['hotel_id']; 
         $test = $this->Breakfast_model->getMasterHotelAvailabilityData($hotel_id, $date);
    ...
    case "single_calender":
        ...
        $food_type = $_POST['type_id'];
            $master = $this->Breakfast_model->getMasterHotelAvailabilityData($hotel_id, $date);
            $type = $this->Breakfast_model->getHotelAvailabilityData($hotel_id, $food_type, $date);
