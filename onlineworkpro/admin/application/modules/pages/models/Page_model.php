...
class Page_model extends MY_Model {
    ...
    public $rules = array(
		...
		'display_on_menu' => array('field'=>'display_on_menu[]', 'label'=>'lang:display_on_menu', 'rules'=>'trim|xss_clean'),
   ...
   public $rules_news = array(
		...
		'template' => array('field'=>'template', 'label'=>'lang:Template', 'rules'=>'trim|intval'),
        'language_id' => array('field'=>'language_id', 'label'=>'lang:Language', 'rules'=>'trim|intval')
   );
		...
		if (count($pages)) {
			foreach ($pages as $order => $page) {
				if ($page['item_id'] != '') {
					...
					$this->db->set($data)->where($this->primary_key, $page['item_id'])->update($this->table_name);
				}
			}
		}
	}
...
//                           'contact' => lang('Contact page'));
        ...
        $templatesDirectory = opendir(BASEPATH.'templates/'.$CI->app_settings['template']);
        // get each template
        while($tempFile = readdir($templatesDirectory)) {
            if ($tempFile != "." && $tempFile != "..") {
                if(substr_count($tempFile, $template_prefix) > 0)
                {
                    $templates[substr($tempFile,0,-4)] = (ucfirst(substr($tempFile, strlen($template_prefix), -4)));
                }
    ...
    public function get_lang($id = NULL, $single = FALSE, $lang_id=1, $where = null, $limit = null, $offset = "", $order_by=NULL)
    {
        if($id != NULL)
        {
            ...
            $this->db->select('*');
            $this->db->from($this->table_name.'_lang');
        ...
        $this->db->join($this->table_name.'_lang', $this->table_name.'.id = '.$this->table_name.'_lang.page_id');
    ...
    public function save_with_lang($data, $data_lang, $id = NULL)
    {
        ...
        // Save lang data
        $this->db->delete($this->table_name.'_lang', array('page_id' => $id));
        ...
        foreach($this->languages as $lang_key=>$lang_val){
            if(is_numeric($lang_key)){
                ...
                $this->db->insert($this->table_name.'_lang');
            }
        }
    ...
    public function get_no_parents($lang_id=1){
        ...
        $this->db->join($this->table_name.'_lang', $this->table_name.'.id = '.$this->table_name.'_lang.page_id');
        $this->db->not_like($this->table_name.'.type', 'MODULE_', 'after');
        $this->db->where('language_id', $lang_id);
        $pages = parent::get();
    ...
    public function get_no_parents_news($lang_id=1, $not_selected = 'No container')
	{
        ...
        $this->db->join($this->table_name.'_lang', $this->table_name.'.id = '.$this->table_name.'_lang.page_id');
        $this->db->where('language_id', $lang_id);
        $this->db->where($this->table_name.'.type !=', 'ARTICLE');
        $this->db->not_like($this->table_name.'.type', 'MODULE_', 'after');
        $this->db->order_by('order, id');
        $pages = parent::get();
        
        // Return key => value pair array
        $array = array(0 => lang($not_selected));
        if(count($pages)){
            foreach($pages as $page){
                if($page->parent_id == 0){
                    $array[$page->id] = $page->title;
    ...
    public function get_no_parents_news_category($lang_id=1)
	{
        ...
        $this->db->join($this->table_name.'_lang', $this->table_name.'.id = '.$this->table_name.'_lang.page_id');
        $this->db->where('language_id', $lang_id);
        $this->db->where($this->table_name.'.type', 'MODULE_NEWS_CATEGORY');
        $this->db->order_by('order, id');
        
        $pages = parent::get();
        
        // Return key => value pair array
        $array = array(0 => lang('No category'));
        if(count($pages))
        {
            foreach($pages as $page)
            {
    ...
    public function delete($id){
		...
		$this->db->set(array(
			'parent_id' => 0
		))->where('parent_id', $id)->update($this->table_name);
    }
